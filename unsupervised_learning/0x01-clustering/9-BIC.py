...
def BIC(X, kmin=1, kmax=None, iterations=1000, tol=1e-5, verbose=False):
    ...
    - iterations: positive integer containing the maximum number of iterations for the EM algorithm
    - tol: non-negative float containing the tolerance for the EM algorithm
    - verbose: boolean that determines if the EM algorithm should print information to the standard output

    Returns:
    - best_k: best value for k based on its BIC
    - best_result: tuple containing pi, m, S
    - pi: numpy.ndarray of shape (k,) containing the cluster priors for the best number of clusters
    - m: numpy.ndarray of shape (k, d) containing the centroid means for the best number of clusters
    - S: numpy.ndarray of shape (k, d, d) containing the covariance matrices for the best number of clusters
    - l: numpy.ndarray of shape (kmax - kmin + 1) containing the log likelihood for each cluster size tested
    - b: numpy.ndarray of shape (kmax - kmin + 1) containing the BIC value for each cluster size tested
    """
